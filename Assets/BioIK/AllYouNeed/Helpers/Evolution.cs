...
namespace BioIK {
	...
	public class Evolution {		
...
#endif
#if !UNITY_EDITOR
                EliteThreads[index] = Task.Factory.StartNew(() => Survive(index));
#endif
            }
            for (int i=0; i<Elites; i++) {
				Work[i] = false;
		...
		public bool Evolve() {
            ...
            //Wait for threads to finish - ORIGINAL
            //            while (Jobs > 0) {
            //                //Debug.Log("Jobs = " + Jobs);
            //#if !UNITY_EDITOR
            //                Debug.Log("");
            //#endif
            //            }
            //Wait for threads to finish - Using the HasJobs() Function
            while (HasJobs())
            {
