...
public class OccupancyGridCustomDots : BaseVisualFactory<OccupancyGridMsg>
{
    ...
    public class OccupancyGridVisual : IVisual
    {
        ...
        Color32 unoccupiedColor = new Color32(byte.MinValue, byte.MaxValue, byte.MinValue, byte.MaxValue);
        Color32 unknownColor = new Color32(byte.MinValue, byte.MinValue, byte.MaxValue, Convert.ToByte(50));
        Color32 occupiedColor = new Color32(byte.MaxValue, byte.MinValue, byte.MinValue, byte.MaxValue);
        ...
        public void Redraw()
        {
            ...
            for (int i = 0; i < m_Message.info.height; i += stepInt)
            {
                for (int j = 0; j < m_Message.info.width; j += stepInt)
                {
                    ...
                    { 
                        dotColor = unknownColor;
                    }
                    else if (m_Message.data[i * m_Message.info.width + j] < 5)
                    {
                        dotColor = unoccupiedColor;
                    }
                    else 
                    {
                        dotColor = occupiedColor;
                    }
                    Vector3 spherePos = drawOrigin + new Vector3(-i * scale, 0.1f, j * scale);
                    m_Drawing.DrawPoint(spherePos, dotColor, m_DotSize);
                }
            }
